...
class OorboekjeParser(HTMLParser):
    ...
    def setup(self):
        ...
        now = time.localtime()
        self.previous_programme_time = time.mktime(
    ...
    def oortime_to_timestamp(self, oor):
        ...
        return time.mktime(
            (
                self.target.tm_year,
                self.target.tm_mon,
                self.target.tm_mday + self.oor_add_days,
                int(hourmin[0]),
                int(hourmin[1]),
                0,
                self.target.tm_wday,
                self.target.tm_yday,
                self.target.tm_isdst,
            )
        )
    ...
    def finish_programme(self):
        if type(self.current_programme_time) == str and "-" in self.current_programme_time:
            ...
            self.xmltvdoc.addProgramme(
                self.current_channel,
                self.current_programme_title,
                self.oortime_to_timestamp(times[0].strip()),
                self.oortime_to_timestamp(times[1].strip()),
                description=self.current_programme_descr,
            )
            # clear out!
            self.setup()
            ...
            self.xmltvdoc.addProgramme(
                self.current_channel,
                self.previous_programme_title,
                self.previous_programme_time,
                self.current_programme_time,
                description=self.previous_programme_descr,
            )
