...
class HorizonRequest(object):
    ...
    def request(self, method, path, retry=True):
        ...
        if response.status == 500:
            debug(
                "Failed to request data from Horizon API, HTTP status {:0}".format(response.status)
            )
...
class ChannelMap(object):
    ...
    def __init__(self):
        ...
        if response:
            ...
            raise Exception(
                "Failed to get data from Horizon API, HTTP status {:d}".format(response.status)
            )
        # load json
...
def parse(raw, xmltv):
    ...
    for listing in data["listings"]:
        ...
        xmltv.addProgramme(
            channel_id, title, start, end, episode, episode_title, description, categories
        )
