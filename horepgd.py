        ...
        for channel_id, channel in chmap.channel_map.items():
            if channel["title"].lower() in (channel.lower() for channel in wanted_channels):
                ...
                if output_folder:
                    ...
                    with open(
                        os.path.join(output_folder, "{}.xml".format(channel_name)), "wb"
                    ) as fd:
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetches EPG data from the Horizon service and passes it to TVHeadend."
    )
    parser.add_argument(
        "-s",
        nargs="?",
        ...
        type=str,
        help="path to TVHeadend XMLTV socket",
    )
    parser.add_argument(
        "-p",
        nargs="?",
        metavar="PATH",
        dest="pid_filename",
        default="/var/run/horepgd.pid",
        type=str,
        help="path to PID file",
    )
    parser.add_argument(
        "-u", nargs="?", metavar="USER", dest="as_user", default="hts", type=str, help="run as USER"
    )
    parser.add_argument(
        "-g",
        nargs="?",
    ...
    parser.add_argument(
        "-d", dest="daemonize", action="store_const", const=True, default=False, help="daemonize"
    )
    parser.add_argument(
        "-1",
        dest="single_shot",
    ...
    parser.add_argument(
        ...
        metavar="HOST",
        default="localhost",
        help="the hostname of TVHeadend to fetch channels from",
    )
    parser.add_argument(
        "-tvh_username",
        dest="tvh_username",
        ...
        type=str,
        default="",
        help="the username used to login into TVHeadend",
    )
    parser.add_argument(
        "-tvh_password",
        dest="tvh_password",
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="so not send to TVHeadend but store to disk in OUTPUTPATH",
    )
    ...
    if args.daemonize:
        ...
        try:
            ...
            debug(
                "Unable to find the user {0} and group {1} for daemonization".format(
                    args.as_user, args.as_group
                )
            )
            sys.exit(1)
    ...
    channels = tvh_get_channels(
        args.tvh_host, username=args.tvh_username, password=args.tvh_password
    )
